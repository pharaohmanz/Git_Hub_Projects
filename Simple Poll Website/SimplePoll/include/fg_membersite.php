...
class FGMembersite
{
    ...
    {
        $this->sitename = 'http://cisvm-cen-26.ccec.unf.edu/';
        $this->rand_key = 'wny3as4Wx9zmHpd';
    }
    ...
    {
	return isset($_SESSION['is_user_admin'])?$_SESSION['is_user_admin']:'';
    }
...
//        }
    ...
    function CheckLoginInDB($username,$password)
    {
        ...
        $pwdmd5 = md5($password);
        $qry = "Select first_name, last_name, email, admin from $this->tablename where email='$username' and password='$pwdmd5'";
	...
	$_SESSION['is_user_admin'] = $row['admin'];
	...
	$result = mysql_query("Select id_user, first_name, last_name, email, state, birth_month, birth_day, birth_year from $this->tablename",$this->connection); 
	return $result;
    }
	...
	$formvars['month'] = $this->Sanitize($_POST['month']);
	$formvars['day'] = $this->Sanitize($_POST['day']);
    ...
    function InsertIntoDB(&$formvars)
    {
		...
		birth_year
			...
			"' . $this->SanitizeForSQL($formvars['month']) . '",
			"' . $this->SanitizeForSQL($formvars['day']) . '",
